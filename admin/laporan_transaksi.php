...
while($stat = mysqli_fetch_assoc($q)) {
    $total_pendapatan += $stat['subtotal'];
    if($stat['status'] == 'pending') $count_pending++;
    if($stat['status'] == 'paid') $count_selesai++;
    if($stat['status'] == 'cancel') $count_canceled++;
}
mysqli_data_seek($q, 0);
?>
<!-- Bootstrap Icons -->
...
<section class="laporan-section">
    ...
    <div class="section-header">
        <div class="header-title">
            ...
            <p class="header-subtitle">Pantau dan analisis data Transaksi café Anda</p>
        </div>
...
/* Customer cell (kalau ada avatar) */
.customer-cell {
    display: flex;
    align-items: center;
    justify-content: center;
